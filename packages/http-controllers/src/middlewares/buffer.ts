 ...
 */
export interface IBufferOptions extends IBodyParserMiddlewareOptions {
}

/**
 * Creates a new middleware, which reads input data as `Buffer`
 * and writes this data to `body` property of the current request context.
 *
 * @param {Nilable<IBufferOptions>} [options] Custom options.
 *
 * @returns {HttpMiddleware<any, any>} The new middleware.
 */
export function buffer(options?: Nilable<IBufferOptions>): HttpMiddleware<any, any> {
